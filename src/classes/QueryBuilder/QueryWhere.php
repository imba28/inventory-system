...
class QueryWhere
{
    ...
    public function __construct(array $where)
    {
        ...
        if (is_array($where[0])) {
            $where[0] = new QueryWhere($where[0]);
        }
        if (is_array($where[2])) {
            $where[2] = new QueryWhere($where[2]);
        }
    ...
    {
        if (count($where) != 3) {
            throw new \InvalidArgumentException("Bedingungen für Where-Clause sind ungültig!");
        }

        if ((is_array($where[0]) && count($where[0]) < 0) || ($where[0] != "0" && empty($where[0]))) {
            throw new \InvalidArgumentException("Argument 1 `$where[0]` für Where-Clause ist ungültig!");
        }
        if ((is_array($where[2]) && count($where[2]) < 0) /*|| ($where[2] != "0" && empty($where[2]))*/) {
            throw new \InvalidArgumentException("Argument 2 `$where[2]` für Where-Clause ist ungültig!");
        }
        if (!in_array(strtoupper($where[1]), QueryWhere::$valid_operators)) {
            throw new \InvalidArgumentException("Operator `$where[1]` für Where-Clause ist ungültig!");
        }
    }
    ...
    public function getValue()
    {
        if ($this->argument_1 instanceof QueryWhere || $this->argument_2 instanceof QueryWhere) {
            ...
            if ($this->argument_1 instanceof QueryWhere) {
                foreach ($this->argument_1->getValue() as $v) {
                    if ($v !== false) {
                        $ret[] = $v;
                    }
                }
            } elseif (!is_null($this->argument_1) && $this->argument_1 !== "NULL") {
                $ret[] = $this->argument_1;
            }

            if ($this->argument_2 instanceof QueryWhere) {
                foreach ($this->argument_2->getValue() as $v) {
                    if ($v !== false) {
                        $ret[] = $v;
                    }
                }
            } elseif (!is_null($this->argument_2) && $this->argument_2 !== "NULL") {
                $ret[] = $this->argument_2;
            }

            return $ret;
        } elseif (!is_null($this->argument_2) && $this->argument_2 !== "NULL") {
            return array($this->argument_2);
        }
    }
    ...
    final private static function sanitize($value, $table)
    {
